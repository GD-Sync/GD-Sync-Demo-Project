...
func _ready() -> void:
	...
	GDSync.expose_func(play_explosion_effect)
	
	_explosion_start_timer.timeout.connect(_explode)

func _physics_process(delta) -> void:
#	Only move the grenade on the host client
...
func _explode() -> void:
#	Play the explosion effect for yourself
	play_explosion_effect()
	
#	Play the explosion effect on all other clients
	GDSync.call_func(play_explosion_effect)
